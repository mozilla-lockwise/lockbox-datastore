...
class ItemKeyStore {
  ...
  constructor(self) {
    ...
    self.iterations = self.iterations || DEFAULT_ITERATIONS;
    self.salt = self.salt ||
                jose.util.base64url.encode(jose.util.randomBytes(DEFAULT_SALTLEN));
  }
   ...
   * @readonly
   */
  get iterations() { return instance.get(this).iterations; }
  /**
   ...
   * @readonly
   */
  get salt() { return instance.get(this).salt; }
  /**
   ...
   * * `encrypted` {**String**} The encrypted map of keys
   *
   * @returns {Object} An object with the following properties: `group`, `salt`,
   *                   `iterations`, and `encrypted`.
   */
  toJSON() {
    return {
      group: this.group,
  ...
  async save() {
    ...
    // calculate next salt
    salt = jose.util.base64url.decode(salt);
    salt = await jose.JWA.digest("SHA-256", salt);
    salt = jose.util.base64url.encode(salt);
