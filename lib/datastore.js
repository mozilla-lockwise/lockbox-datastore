...
const PBKDF2_DEFAULT_ITERATIONS = 8192,
      PBKDF2_DEFAULT_SALTLEN = 32;

async function _encryptItemKeys(self, keys) {
  let encrypted = {};
  ...
  encrytped = JSON.stringify(encrypted);

  // calculate salt and iterations
  let params = self.pbkdf2 = self.pbkdf2 || {};
  let iterations = params.iterations || PBKDF2_DEFAULT_ITERATIONS;
  let salt = params.salt || jose.util.randomBytes(PBKDF2_DEFAULT_SALTLEN);
  encrypted = await jose.JWE.createEncrypt({
    format: "compact",
    contentAlg: "A256GCM",
    fields: {
  ...
  }, masterKey).final(encrypted, "utf8");
  // calculate next salt
  params.salt = await jose.JWA.digest("SHA-256", salt);
...
class DataStore {
  ...
  async initialize(opts) {
    ...
    let keystore = _encryptItemKeys(self, itemKeys);
