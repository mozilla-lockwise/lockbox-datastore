...
class DataStore {
  ...
  async initialize(opts) {
    ...
    let masterKey = await jose.JWK.asKey({
      kty: "oct",
      k: jose.util.base64url.encode(password, "utf8"),
      use: "enc"
    });
    ...
    self.keystore = await keystore.load();
  ...
  async unlock() {
    ...
    while (!keystore.masterKey) {
      ...
      try {
        ...
        let pwd = await prompts.unlock(req);
