...
class DataStore {
  ...
  async initialize(opts) {
    ...
    if (self.keystore.encrypted) {
      if (!opts.rebase) {
        ...
        throw new DataStoreError(DataStoreError.LOCKED, "must to unlocked in order to rebase");
      }
  ...
  async update(item) {
    ...
    if (!item || !item.id) {
      // TODO: custom errors
      throw new DataStoreError(DataStoreError.INVALID_ITEM, "invalid item");
    }
    ...
    let orig = await self.ldb.items.get(id),
        encrypted;
    if (!orig) {
      throw new DataStoreError(DataStoreError.MISSING_ITEM, "item does not exist");
    } else {
